...
import { GraduationCap, FileDown, Plus, Trash2, Clock, Check, Database } from 'lucide-react';
...
function App() {
  ...
  const generatePDF = async () => {
    ...
    if (notesData.actionItems.length > 0) {
      const validItems = notesData.actionItems.filter(item => item.text.trim());
  ...
  const deleteRecord = (id: string) => {
    const updatedRecords = clientRecords.filter(record => record.id !== id);
    setClientRecords(updatedRecords);
    localStorage.setItem('stwClientRecords', JSON.stringify(updatedRecords));
  };

  if (currentView === 'timetracker') {
    return <TimeTracker onBack={() => setCurrentView('notes')} onSaveInvoice={saveInvoiceRecord} />;
  }

  if (currentView === 'crm') {
    return <CRMRecords onBack={() => setCurrentView('notes')} records={clientRecords} onDeleteRecord={deleteRecord} />;
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNotesData(prev => ({ ...prev, studentName: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNotesData(prev => ({ ...prev, date: e.target.value }))}
                    className="w-full p-4 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-900/30 focus:border-blue-900 transition-all duration-200 font-medium text-slate-800 bg-white"
                  />
                </div>
              </div>
            ...
            <div className="mb-8">
              ...
              <div className="bg-slate-50/50 rounded-lg border border-slate-200 p-1">
                <textarea
                  ...
                  onChange={(e) => setNotesData(prev => ({ ...prev, conversationPoints: e.target.value }))}
            ...
            <div className="mb-8">
              ...
              <div className="bg-blue-50/50 rounded-lg border border-blue-200 p-1">
                <textarea
                  ...
                  onChange={(e) => setNotesData(prev => ({ ...prev, topicsCovered: e.target.value }))}
                  className="w-full p-4 border-0 rounded-lg focus:ring-2 focus:ring-blue-900/30 resize-none bg-white text-slate-700 leading-relaxed"
                  rows={3}
            ...
            <div className="mb-8">
              ...
              <div className="bg-slate-50/30 rounded-lg border border-slate-200 p-1">
                <textarea
                  ...
                  onChange={(e) => setNotesData(prev => ({ ...prev, goals: e.target.value }))}
                  className="w-full p-4 border-0 rounded-lg focus:ring-2 focus:ring-blue-900/30 resize-none bg-white text-slate-700 leading-relaxed"
                  rows={3}
